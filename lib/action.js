...
export async function avramAction(args, opt) {
  ...
  let schemaFile = ((opt.schema || opt.document) && !files.length) ? "-" : files.shift()
  if (schemaFile === "-") {
    schemaFile = "/dev/stdin"
  }
  if (schemaFile === undefined) {
    throw new Error("Missing schema argument")
  }
...
export async function validateFiles(files, schema, validationOptions, opt) {
  const { format, ignore, verbose, print } = opt
 
  validationOptions["ignore"] = ignore
