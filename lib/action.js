...
export async function avramAction(args, opt) {
  ...
  // load extensions
  opt.extension = (opt.extension||"").split(",").map(e => {
    if (e === "marc" || e === "marc21") {
...
// opt supports: type, format, print, verbose
export async function validateFiles(files, schema, validationOptions, opt) {
  ...
  await Promise.all(files.map(file => new Promise((resolve, reject) => {
    ...
    const stream = inputFormat.stream(input) // TODO: marcxml parser should emit error on parsing error
