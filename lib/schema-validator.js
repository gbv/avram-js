    ...
    for (let id in fields) {
      ...
      for (let ind of ["indicator1","indicator2"]) {
        if (typeof field[ind]?.codes == "object") {
          ...
          errors.push(...this.validateCodes(field[ind].codes || {}, path))
        }
      }
      if (field.subfields) {
  ...
  validatePositions(positions, path) {
    ...
    for (let pos in positions) {
      ...
      if ("start" in p && p.start != start) {
        errors.push({ message: `position start '${p.start}' must be '${start}'`, position: pos, ...path })
      }
      if ("end" in p && p.end != end) {
        errors.push({ message: `position end '${p.end}' must be '${end}'`, position: pos, ...path })
      }
