...
export class FieldIdentifier {
  ...
  match(field) {
    ...
    if (tag === this.tag) {
      if (this.occurrence) {
        return this.occurrence.match(occurrence)
...
export class FieldSchedule {
  ...
  getIdentifier(field) {
    // TODO: this is slow, better create a lookup table
    for (let id in this.schedule) {
      const idObject = FieldIdentifier(id)
  ...
  getDefinition(field) {
    return this.schedule[this.getIdentifier(field)]
  }
}
