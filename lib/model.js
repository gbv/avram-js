  ...
  pica: {
    ...
    identifier(id) {
      ...
      if (tag === id) {
        ...
        if (tag.charAt(0) === "2") {
          if (id.substr(5,2) === "$x") {
            return { tag, counter: new Range(id.substr(7)) }
          } else {
...
export class FieldSchedule {
  constructor({ family, fields }) {
    ...
    for (let id in fields) {
      // TODO: add tag and occurrence to field definitions
      fieldIds[id] = new FieldIdentifier(id, family)
    }

    const identifier = field => {
      // FIXME: this is slow, better create a lookup table by tag
      for (let id in fieldIds) {
        if (fieldIds[id].match(field)) {
          return id
        }
      }
    }
