  ...
  pica: {
    ...
    identifier(id) {
      ...
      if (key === id) {
        ...
        return { key, counter: new Range(id.substr(5)) }
      }
    },
  },
  flat: {
...
export class FieldIdentifier {
  ...
  constructor(id, family) {
    ...
    const { key, identifier } = families[family]
    if (identifier) {
      try {
  ...
  match(field) {
    const { key, occurrence, subfields } = field
...
export class FieldSchedule {
  constructor({ family, fields }) {
    ...
    const identifier = field => {
      // FIXME: this is slow, better create a lookup table by key
