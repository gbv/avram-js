...
export class Validator {
  ...
  constructor(schema, options={}, extensions=[]) {
    ...
    this.ignoreFields = this.scheduleFromFieldlist(options.ignore)
  }

  scheduleFromFieldlist(fields) {
    if (fields) {
      fields = Object.fromEntries(fields.split(",").map(id => [id,{}])) 
      return new FieldSchedule({ family: this.family, fields })
    }
  }
  ...
  validateRecords(records, options={}) {
    ...
    const ignoreFields = "ignore" in options ? this.scheduleFromFieldlist(options.ignore) : this.ignoreFields
    ...
    for (let record of records) {
      ...
      for (let field of fields) {
        if (ignoreFields && ignoreFields.identifier(field) in ignoreFields) {
          continue
        }
...
const validateFieldContent = (field, definition, options, types, id) => {
  ...
  if (field.occurrence) {
    path.occurrence = field.occurrence
  }
...
const validateIndicators = (field, definition, options, id) => {
  ...
  for (let indicator of ["indicator1", "indicator2"]) {
    ...
    if (indicator in field !== indicator in definition) {
      ...
      if (codes) {
        errors.push(...validateCode(field[indicator], codes, options, path))
      }
      if (pattern) {
        errors.push(...validatePattern(field[indicator], pattern, path))
      }
    }
  }
  return errors
...
const validateCode = (value, codes, options = {}, path = {}, len=null) => {
  ...
  if (!codes || (!undefinedCode && !isIndicator)) {
    return []
  }
...
export const validatePositions = (value, positions, options = {}, path)  => {    
  if (!positions || !options.invalidPosition) {
    return []
  }
