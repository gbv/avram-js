      ...
      for (let field of record) {
        ...
        if (id) {
          ...
          if (!fieldVisitor.visit(id)) {
            ...
            errors.push({ message, identifier: id, violates: "AR3" })
          }
          ...
          errors.push({ ...error, violates: "AR2" })
        }
      }
...
const validateFieldContent = (field, definition, options) => {
  ...
  // TODO: validate indicators (indicator1, indicator2): AR11
...
export const validateSubfields = (subfields, schedule, options) => {
  ...
  const missing = visitor.missing().map(code => ({
    message: `Missing subfield '${code}'.`, code, violates: "AR9",
  }))
  errors.push(...missing)
