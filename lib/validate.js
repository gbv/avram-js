        ...
        if (options.invalidRecord) {
          if (id) {
            ...
            if (definition.deprecated && options.deprecatedField) {
              errors.push({
                message: `Field '${id}' is deprecated`, id,
                error: "deprecatedField",
              })
            } else {
              errors.push(
                ...validateFieldContent(field, definition, options, id, types),
...
const validateFieldContent = (field, definition, options, id, types) => {
  if (Array.isArray(field.subfields)) {    // variable field
    ...
    return validateValue(field.value, definition, options, types)
  } else {
    return []
  }
}
...
export const validateSubfields = (subfields, schedule, options, id) => {
    ...
    if (code in schedule) {
      if (options.deprecatedSubfield && definition.deprecated) {
        errors.push({
          ...
          code, id,
          message: `Subfield '${code}' in field '${id}' is deprecated`,
        })
      } else { 
        if (!visitor.visit(code) && options.nonrepeatableSubfield) {
          errors.push({
            message: `Subfield '${code}' in field '${id}' must not be repeated.`,
        ...
        if (options.invalidSubfieldValue) {
          ...
          errors.push(...validateValue(value, definition, options))
        }
      }
    } else if(options.undefinedSubfield) {
      errors.push({
        message: `Unknown subfield '${code}' in field '${id}'.`,
        code, id,
  ...
  if (options.missingSubfield) {
    errors.push(...visitor.missing().map(code => ({
      message: `Missing subfield '${code}' in field '${id}'.`,
      code, id, error: "missingSubfield",
    })))
  }
...
export const validateValue = (value, definition, options, recordTypes) => {
  const valueErrors = ({pattern, positions, codes}) => [
    ...
    ...validateCode(value, codes, options),
  ]
...
export const validateCode = (value, codes, options = {}, isIndicator = false) => {
